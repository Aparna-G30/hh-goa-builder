...
const BuilderCard = forwardRef(function BuilderCard(
  ...
  return (
    <motion.div
      ...
      className={`mx-auto ${SIZE_CLASSES[size] || SIZE_CLASSES.default}`}
      style={{ aspectRatio: "2 / 3" }}
    >
      <div
        ref={ref}
        className="sticker relative flex h-full w-full flex-col overflow-hidden rounded-[32px] border-4 border-[#063B2A] bg-gradient-to-br from-[#FFF4D6] via-[#FFF4D6] to-[#FFE8AE] p-6 text-[#111111] shadow-2xl"
      >
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-5 py-4">
          {builder?.image ? (
            <img
              ...
              className="h-44 w-36 rounded-[20px] border-2 border-[#063B2A]/20 object-cover shadow-lg sm:h-52 sm:w-40"
            />
          ) : (
            <div className="flex h-44 w-36 items-center justify-center rounded-[20px] border-2 border-dashed border-[#087F4F]/40 bg-[#dfe8d5] text-sm text-[#087F4F]/70 shadow-inner sm:h-52 sm:w-40">
              Photo
            </div>
          )}
